...
const TITLES = { '\u7d66\u4e0e':'\u266b \u7a3c\u3052\u308b\u74b0\u5883','\u5ba2\u8cea':'\u2661 \u7d33\u58eb\u69d8\u591a\u6570','\u4eba\u9593':'\u2726 \u5c45\u5fc3\u5730\u629c\u7fa4','\u6c42\u4eba':'\u2727 \u5618\u306a\u3057\u5ba3\u8a00','\u901a\u52e4':'\u2606 \u30a2\u30af\u30bb\u30b9\u25ce','\u5f85\u6a5f':'\u2726 \u5feb\u9069\u7a7a\u9593' }
function getTitle(c){if(!c)return'\u2726 Real Voice';for(var k in TITLES){if(c.indexOf(k)!==-1)return TITLES[k]}return'\u2726 Real Voice'}
function extractHL(b){if(!b)return'';var s=b.split(/[\u3002\uff01!]/g).filter(function(x){return x.trim().length>4});if(!s.length)return b.slice(0,60);var sh=s.filter(function(x){return x.trim().length<=30});return(sh.length?sh[0]:s[0]).trim()}
...
export default async function handler(req) {
  ...
  if (mode === 'html') {
    ...
    var desc = rv ? (rv.body||'').slice(0,120) : 'OKINI Tokyo\u306e\u30ea\u30a2\u30eb\u306a\u53e3\u30b3\u30df'
    var t = rv ? 'OKINI Tokyo | '+(rv.category||'Real Voice') : 'OKINI Tokyo | Real Voice'
    return new Response(
  ...
  return new ImageResponse(
    ({
      type:'div',props:{style:{display:'flex',flexDirection:'column',width:'100%',height:'100%',background:'#0f0d18',fontFamily:'sans-serif',color:'#fff',padding:48,position:'relative'},children:[
        ...
        {type:'div',props:{style:{display:'flex',alignItems:'center',gap:12,marginBottom:24},children:[
        ...
        {type:'div',props:{style:{display:'flex',alignItems:'center',background:'linear-gradient(90deg,rgba(201,168,76,0.22),rgba(201,168,76,0.08))',border:'2px solid rgba(201,168,76,0.28)',borderRadius:8,padding:'24px 28px',marginBottom:16},children:[
        ...
        {type:'div',props:{style:{display:'flex',position:'absolute',bottom:0,left:0,width:'100%',height:3,background:'linear-gradient(90deg,rgba(201,168,76,0.5),rgba(232,69,124,0.4),transparent)'}}}
      ]}
    }),
