...
export default function SharePanel({ review }) {
  ...
  const twitterUrl = `https://twitter.com/intent/tweet?text=${encodeURIComponent(shareText)}&url=${encodeURIComponent(reviewUrl)}`
  ...
  const handleDownloadCard = (format) => {
    // OGP画像URLを開く（Vercelデプロイ後に動作）
  ...
  return (
    <div style={{ marginTop: 16 }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
        {/* X Share */}
        <a href={twitterUrl} target="_blank" rel="noopener noreferrer" style={shareBtn('#1DA1F2')}>
      ...
      <div style={{ marginTop: 16, background: 'rgba(255,255,255,0.02)', border: '1px solid var(--bdr)', borderRadius: 12, padding: 14, borderLeft: `3px solid ${catColor(review.category)}` }}>
        ...
        <p style={{ fontSize: '0.78rem', color: 'var(--td)', lineHeight: 1.6, whiteSpace: 'pre-wrap' }}>{shareText}</p>
        <p style={{ fontSize: '0.68rem', color: 'var(--blue)', marginTop: 4 }}>{reviewUrl}</p>
      </div>
    </div>
  )
}
...
const shareBtn = (bg) => ({
  ...
  fontFamily: 'var(--f)', transition: 'all 0.2s', width: '100%', textDecoration: 'none',
})
